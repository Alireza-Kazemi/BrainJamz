...
data_Corr      = read.csv(paste(RD,"Word_PermMESameDiff_ForR.csv",sep=""),sep = ",",header=TRUE,strip.white=TRUE)
data_Corr$Conditions = factor(data_Corr$Conditions, 
                              levels = c("target", "known", "unknown", "baseline", 
                                         "target_known", "target_unknown", "known_unknown",
                                         "target_baseline", "known_baseline", "unknown_baseline"))
...
############################################################################## Perm Micro Event Separated Blocks Same/Different Words ----
########################### Word PermMeSameDiff Within Condition All Masks ##########################
sDat = WordPMESDB
unique(sDat$Conditions)
sDat = sDat[sDat$Conditions %in% c("target", "known", "unknown"),]
sDat$PermCond = paste(sDat$Perm1,sDat$Perm2,sep = "_")
