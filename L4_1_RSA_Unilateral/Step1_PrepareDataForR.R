...
########################### Prepare Song PermMicroEventsS Separated Blocks Data and Save #################
data_Corr      = read.csv(paste(RD,"RSA_PermMicroEventsS_Song.csv",sep=""),sep = ",",header=TRUE,strip.white=TRUE)
songTrack      = read.csv(paste(RD,"TrackTable.csv",sep=""),sep = ",",header=TRUE,strip.white=TRUE)
songTrack$Subj = songTrack$ID
data_Corr = merge(data_Corr,songTrack[,c("Subj","songGroup")],by = "Subj")
...
A = as.data.frame(strsplit(unique(data_Corr$Conditions),"_"))
A = transpose(A)
names(A) = c("Cond1","T1","Perm1","Cond2","T2","Perm2")
A$Conditions = paste(A$Cond1,A$T1,A$Perm1,A$Cond2,A$T2,A$Perm2,sep="_")
...
data_Corr$Mask = gsub("_L","",data_Corr$Mask)
data_Corr$Mask = gsub("_R","",data_Corr$Mask)
write.csv(data_Corr,"Song_PermMicroEventsS_ForR.csv", row.names = F)


########################### Prepare Word PermMicroEventsS Separated Blocks Data and Save #################
data_Corr      = read.csv(paste(RD,"RSA_PermMicroEventsS_Word.csv",sep=""),sep = ",",header=TRUE,strip.white=TRUE)
