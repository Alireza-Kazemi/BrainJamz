...
########################### Load and Save Wide FormatData -------
dat      = read.csv(paste(RD,"WordCollapsed_Connectivity_Sig_ForR.csv",sep=""),sep = ",",header=TRUE,strip.white=TRUE)
...
unique(sDat$Regions)
head(sDat)
sDat$connStr = unique("ConnScore")

SaveDat = reshape2::dcast(sDat,Subj~connStr+Condition+Regions+Hemisphere,value.var = "CorrAllS_Sig")
write.csv(SaveDat,"ConnectivityScores.csv",row.names = F)
########################### LoadData #################
dat      = read.csv(paste(RD,"WordCollapsed_Connectivity_Sig_ForR.csv",sep=""),sep = ",",header=TRUE,strip.white=TRUE)
dat$Condition = factor(dat$Condition, 
                              levels = c("target" , "known", "unknown", "baseline"))
...
write.csv(SaveDat,"ConnectivityScores.csv")
