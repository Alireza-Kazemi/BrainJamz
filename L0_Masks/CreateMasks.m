...
Atlas = double(niftiread("infant-2yr-aal.nii.gz"));
AtlasInfo = niftiinfo("infant-2yr-aal.nii.gz");
names = fields(Masks);
...
Brain = niftiread("infant-2yr-aal.nii.gz");
...
for maskIdx = 1:length(names)
    niftiwrite(Masks.(names{maskIdx}),string(names{maskIdx}+".nii"), AtlasInfo);
end
